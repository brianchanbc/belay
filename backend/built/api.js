...
const authenticateApiKey = (request, response, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const row = yield dbGet(getUserQuery, [apiKey]);
...
app.post('/api/signup', (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const row = yield dbGet(getUserQuery, [username]);
        // Check if the username is already taken
        if (row) {
            return response.status(400).json({ error: 'Username is already taken' });
        }
        const hashAndSaltedPw = yield bcryptHash(password, salt);
        const apiKey = createAPIKey();
        const insertNewUserQuery = `INSERT INTO users (username, password, api_key) VALUES (?, ?, ?)`;
        yield dbRun(insertNewUserQuery, [username, hashAndSaltedPw, apiKey]);
...
app.get('/api/login', (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    const username = request.headers.username;
    const password = request.headers.password;
    // Type guards
    if (typeof username !== 'string' || typeof password !== 'string') {
    ...
    try {
        ...
        const result = yield bcryptCompare(password, row.password);
        if (result) {
...
app.put('/api/change_profile', authenticateApiKey, (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        if (newUsername) {
            ...
            yield dbRun(changeUsernameQuery, [newUsername, apiKey]);
        }
        if (newPassword) {
            const hashAndSaltedPw = yield bcryptHash(newPassword, salt);
            const changePasswordQuery = `UPDATE users SET password = ? WHERE api_key = ?`;
            yield dbRun(changePasswordQuery, [hashAndSaltedPw, apiKey]);
        }
...
app.post('/api/create_channel', authenticateApiKey, (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        yield dbRun(createChannelQuery, [channelName]);
...
// API to get channels
app.get('/api/get_channels', authenticateApiKey, (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const getChannelsQuery = `SELECT * FROM channels`;
        const rows = yield dbAll(getChannelsQuery, []);
        response.json(rows);
    }
    catch (err) {
        console.error(err.message);
...
app.post('/api/create_message/:channelName', authenticateApiKey, (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        // Get channel id
        const channelRow = yield dbGet(`SELECT id FROM channels WHERE name = ?`, [channelName]);
        // Input check
        if (!channelRow) {
        ...
        // Get user id
        const userRow = yield dbGet(`SELECT id FROM users WHERE username = ?`, [username]);
        // Input check
        if (!userRow) {
        ...
        yield dbRun(createMessageQuery, [channelId, userId, messageContent, repliesTo]);
...
app.get('/api/get_messages/:channelid', authenticateApiKey, (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
      ...
      WHERE m.channel_id = ?
    `;
        const rows = yield dbAll(getMessagesQuery, [channelId]);
...
app.post('/api/update_reaction/:message_id', authenticateApiKey, (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const message = yield dbGet(`SELECT id FROM messages WHERE id = ?`, [messageID]);
    if (!message) {
    ...
    try {
        ...
        const existingReaction = yield dbGet(`SELECT emoji FROM reactions WHERE message_id = ? AND user_id = ?`, [messageID, userId]);
        if (existingReaction) {
            if (existingReaction.emoji === emoji) {
                // If the existing reaction is the same as the new one, delete it
                yield dbRun(`DELETE FROM reactions WHERE message_id = ? AND user_id = ?`, [messageID, userId]);
                response.json({ message: 'Reaction deleted successfully' });
            }
            else {
                // If the existing reaction is different from the new one, update it
                yield dbRun(`UPDATE reactions SET emoji = ? WHERE message_id = ? AND user_id = ?`, [emoji, messageID, userId]);
                response.json({ message: 'Reaction updated successfully' });
            }
        }
        else {
            // If no reaction exists, insert a new one
            yield dbRun(`INSERT INTO reactions (message_id, user_id, emoji) VALUES (?, ?, ?)`, [messageID, userId, emoji]);
...
app.put('/api/update_last_seen_message/:channelid', authenticateApiKey, (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const messageRow = yield dbGet(`SELECT id FROM messages WHERE channel_id = ? ORDER BY id DESC LIMIT 1`, [channelid]);
        if (!messageRow) {
        ...
        const userReadMessageRow = yield dbGet(`
...
app.get('/api/get_unread_message_counts/:username', authenticateApiKey, (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    try {
      ...
      GROUP BY c.id
    `;
        const rows = yield dbAll(getUnreadMessageCountsQuery, [userId]);
        response.json(rows);
    }
    catch (err) {
        console.error(err.message);
